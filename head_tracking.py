...
class BodyGameRuntime(object):
        ...
        while not self._done:
            ...
            if self._bodies is not None:
                for i in range(0, self._kinect.max_body_count):
                    ...
                    self.getTblJoints(queue)
                    self.head_tracking(queue)
    ...
    def movement(self, x, y, z, queue):
        c = 20
        queue.put({'translate':
                   [x * c - self.previous_position[0],
                    y * c - self.previous_position[1],
                    -z * c + self.previous_position[2]]})
        self.previous_position = [x * c, y * c, z * c]

    def rotation(self, HP_angle, HY_angle, queue):
        c = 20
        d = 100
        queue.put({'rotate': c * HY_angle - self.previous_angle[0]})
        self.previous_angle = [c * HY_angle, d * HP_angle]

    def getTblJoints(self, queue):
        self.tblJoints = []
        for i in range(0, 25):
            tup = (self.joints[i].Position.x, self.joints[i].Position.z, self.joints[i].Position.y)
            if i == 3:
                self.movement(self.joints[i].Position.x, self.joints[i].Position.y, self.joints[i].Position.z, queue)
    ...
    def head_tracking(self, queue):
        ...
        self.rotation(HPangle, HYangle, queue)
